...
public class DocumentStorageService {
    ...
    public String storeFile(final MultipartFile file, final DocumentStorageVO documentStorageVO, final String docType) {
        final UUID topicId = documentStorageVO.getTopicId();
        final Integer positionIndex = documentStorageVO.getPositionIndex();
        final String note = documentStorageVO.getNote();
        ...
        try {
            ...
            fileName = topicId + "_" + docType + fileExtension;
            // Copy file to the target location (Replacing existing file with the same name)
            Path targetLocation = this.fileStorageLocation.resolve(fileName);
            Files.copy(file.getInputStream(), targetLocation, StandardCopyOption.REPLACE_EXISTING);
            DocumentStorageProperties doc = docStorageRepo.checkDocumentByTopicId(topicId, docType);
            if (doc != null) {
                ...
                newDoc.setTopic(new Topic() {{
                    setId(topicId);
                }});
                newDoc.setDocumentFormat(file.getContentType());
                newDoc.setFileName(fileName);
                newDoc.setDocumentType(docType);
                newDoc.setPositionIndex(positionIndex);
                newDoc.setNote(note);
    ...
    public String getDocumentName(final UUID topicId, final String docType) {
        return docStorageRepo.getUploadDocumentPath(topicId, docType);
    }
}
